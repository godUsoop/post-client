...
const Create = ({token}) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post(`${env.API}/link`, {title, url, categories, type, medium}, {
                headers: {
    ...
    const form = () => {
        return (
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                    <button disabled={!token} className="btn btn-secondary btn-lg">{hasAuth() || token ? "Post" : "Please login"}</button>
                </div>
            </form>
        )
    }
